...
SELECT 'Inserting into validator_market_rate table' AS comment;
...
SELECT tc.height block_height,
       ...
       tc.id tx_hash_id,
       tc.hash tx_hash,
       (tc.tx_v2 -> 'tx' -> 'auth_info' -> 'fee' -> 'amount' -> 0 ->> 'amount')::numeric /
            (tc.tx_v2 -> 'tx_response' ->> 'gas_wanted')::numeric market_rate,
...
SELECT 'Updating block insert procedure' AS comment;
...
BEGIN
    IF NOT EXISTS
        (SELECT 1 FROM pg_type t
         ...
         WHERE t.typname = 'tx_update' AND a.attname = 'valmarketrate')
    THEN
        ALTER TYPE tx_update ADD ATTRIBUTE valMarketRate integer;
    END IF;
END $$;
