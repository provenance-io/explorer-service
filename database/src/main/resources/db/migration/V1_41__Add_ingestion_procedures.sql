...
BEGIN
    ...
    SELECT (tu).txGasFee.hash, (tu).txGasFee.gas_wanted, (tu).txGasFee.gas_used, timez, (tu).txGasFee.fee_amount
    ON CONFLICT (hash) DO NOTHING;
    -- insert fee
    FOREACH tf IN ARRAY (tu).txFees
        LOOP
            INSERT INTO tx_fee(tx_hash, fee_type, marker_id, marker, amount, block_height, tx_hash_id)
            SELECT tf.tx_hash, tf.fee_type, tf.marker_id, tf.marker, tf.amount, tx_height, tx_id
                ...
                LOOP
                    ...
                    FOREACH attr IN ARRAY (event).txAttrs
                        LOOP
                            ...
                            SELECT eventId, attr.attr_key, attr.attr_value, attr.attr_idx, attr.attr_hash
    ...
    FOREACH sm IN ARRAY (tu).singleMsg
        LOOP
            ...
            SELECT sm.tx_hash, timez, sm.gas_used, sm.tx_message_type
    ...
    FOREACH aj IN ARRAY (tu).addressJoin
        LOOP
            ...
            SELECT tx_height, aj.tx_hash, aj.address, tx_id, aj.address_id, aj.address_type
    ...
    FOREACH mj IN ARRAY (tu).markerJoin
        LOOP
            ...
            SELECT tx_height, mj.tx_hash, mj.denom, tx_id, mj.marker_id
    ...
    FOREACH nj IN ARRAY (tu).nftJoin
        LOOP
            INSERT INTO tx_nft_join(block_height, tx_hash, tx_hash_id, metadata_id, metadata_type,
                                    metadata_uuid)
            SELECT tx_height, nj.tx_hash, tx_id, nj.metadata_id, nj.metadata_type, nj.metadata_uuid
    ...
    FOREACH codej IN ARRAY (tu).smCodes
        LOOP
            ...
            SELECT tx_height, codej.tx_hash, tx_id, codej.sm_code
    ...
    FOREACH contractj IN ARRAY (tu).smContracts
        LOOP
            ...
            SELECT tx_height, contractj.tx_hash, tx_id, contractj.sm_contract_id, contractj.sm_contract_address
    ...
    FOREACH sig IN ARRAY (tu).sigs
        LOOP
            ...
            SELECT sig.join_type, tx_id, sig.signature_id
    ...
    FOREACH tfp IN ARRAY (tu).feepayers
        LOOP
            ...
            SELECT tx_height, tx_id, tfp.tx_hash, tfp.payer_type, tfp.address_id, tfp.address
...
BEGIN
    ...
    SELECT tx_height, timez, (bd).blocks.tx_count
    ...
    SELECT tx_height, (bd).validatorCache.validators, (bd).validatorCache.last_hit, (bd).validatorCache.hit_count
    ON CONFLICT (height) DO NOTHING;
