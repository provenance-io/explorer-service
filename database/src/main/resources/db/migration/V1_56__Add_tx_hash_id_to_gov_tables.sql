...
SET tx_hash_id = tc.id
FROM tx_cache tc
WHERE gv.tx_hash = tc.hash;

ALTER TABLE gov_deposit
    ADD COLUMN IF NOT EXISTS tx_hash_id INT NOT NULL DEFAULT 0;

UPDATE gov_deposit gd
SET tx_hash_id = tc.id
FROM tx_cache tc
WHERE gd.tx_hash = tc.hash;

SELECT 'Updating procedures' AS comment;
    ...
    FOREACH gd IN ARRAY (tu).deposits
        LOOP
            INSERT INTO gov_deposit(proposal_id, address_id, address, block_height, tx_hash, tx_timestamp,
                                    is_validator, deposit_type, amount, denom, tx_hash_id)
...
INSERT INTO tx_sm_code (block_height, tx_hash_id, tx_hash, sm_code)
select
...
from proposal_monitor pm
         join sm_code sc on pm.matching_data_hash = sc.data_hash
         join gov_proposal gp on pm.proposal_id = gp.proposal_id
         left join tx_cache tc on tc.hash = gp.tx_hash
