...
@ActiveProfiles("test")
@Transactional
open class ValidatorAddressServiceTest{

    @Autowired
    lateinit var validatorAddressService: ValidatorAddressService
    ...
    fun `should throw KotlinNullPointerException error when searching with invalid consensus address`() {
        validatorAddressService.findAddressesByConsensusAddress("I'm not match worthy")
    }

    @Test(expected=KotlinNullPointerException::class)
    @Ignore("TODO turn into integration tests")
    fun `should throw KotlinNullPointerException error when searching with invalid consensus pub key address`() {
        validatorAddressService.findAddressesByConsensusPubKeyAddress("I'm not match worthy either")
    }

    @Test
    @Ignore("TODO turn into integration tests")
    fun `should return matching operator address from consesus address`(){
        val result = validatorAddressService.findAddressesByConsensusAddress("tpvalcons1pkj6cy0a6acmm3q2augsgkj27h79k6ywqj35gu")
    ...
    fun `should return matching operator address from consesus pubkey address`(){
        val result = validatorAddressService.findAddressesByConsensusPubKeyAddress("tpvalconspub1zcjduepq2hs3lwp8d0u4j0389knqaer4j97quc8v9updrz4yecgkm3uy46ls6rm7ph")
