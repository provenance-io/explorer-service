...
import io.provenance.explorer.domain.TXSearchResult
...
class ExplorerController(private val serviceProperties: ServiceProperties,
    ...
    fun echo(@RequestBody echo: Any): ResponseEntity<Any> = if (user() != serviceProperties.adminUUID.toUuidProv()) {
    ...
    @GetMapping(value = ["/block"],
            produces = [MediaType.APPLICATION_JSON_VALUE])
    fun blockHeight(@RequestParam(required = true) height: Long): ResponseEntity<Any> = if (user() != serviceProperties.adminUUID.toUuidProv()) {
        throw AccessDeniedException("access denied")
    } else {
        ResponseEntity.ok(tendermintService.getBlockAtHeight(height))
    }

    @ApiOperation(value = "Return transaction by hash value")
    @GetMapping(value = ["/tx"],
            produces = [MediaType.APPLICATION_JSON_VALUE])
    fun txByHash(@RequestParam(required = true) hash: String): ResponseEntity<Any> = if (user() != serviceProperties.adminUUID.toUuidProv()) {
        throw AccessDeniedException("access denied")
    } else {
    ...
    fun recentBlocks(@RequestParam(required = true, defaultValue = "10") count: Int, @RequestParam(required = true, defaultValue = "0") page: Int, @RequestParam(required = false, defaultValue = "desc") sort: String):
        ...
        ResponseEntity.ok(tendermintService.getRecentBlocks(count, page, sort))
    }

    @ApiOperation(value = "/validators/?count&page&sort - Get Validators (ex: Top 10 Validators)")
    @GetMapping(value = ["/validators/"],
            produces = [MediaType.APPLICATION_JSON_VALUE])
    fun validators(@RequestParam(required = true) count: Int, @RequestParam(required = false) sort: String): ResponseEntity<Any> = if (user() != serviceProperties.adminUUID.toUuidProv()) {
        throw AccessDeniedException("access denied")
    } else {
    ...
    @GetMapping(value = ["/txs/history"],
            produces = [MediaType.APPLICATION_JSON_VALUE])
    fun txHistory(@RequestParam(required = true) from: String, @RequestParam(required = true) to: Int): ResponseEntity<Any> = if (user() != serviceProperties.adminUUID.toUuidProv()) {
        throw AccessDeniedException("access denied")
    } else {
